...
uint32_t RA8876_common::reverse_nibbles(uint32_t x, size_t size) {
  uint32_t out = 0, i;
  for(i = 0; i < size; ++i)
  {
...
//    lcdRegDataWrite(RA8876_BTE_COLR, RA8876_S0_COLOR_DEPTH_16BPP << 5 | RA8876_S1_COLOR_DEPTH_16BPP << 2 | RA8876_S0_COLOR_DEPTH_16BPP); // 92h
    ...
    // Set initial color depth.
    _color_depth = RA8876_IMAGE_COLOCR_DEPTH_24BPP;
    _s0_color_depth = RA8876_IMAGE_COLOCR_DEPTH_24BPP;
    _s1_color_depth = RA8876_IMAGE_COLOCR_DEPTH_24BPP;
    _dest_color_depth = RA8876_IMAGE_COLOCR_DEPTH_24BPP;
    ...
    _TXTForeColor = COLOR16M_WHITE;
    _TXTBackColor = COLOR16M_DARKBLUE;
    RA8876_BUSY = false;
    //_FNTrender = false;
    /* set-->  _TXTparameters  <--
    0:_extFontRom = false;
    ...
    */
    ...
    fillScreen(COLOR16M_DARKBLUE);  // Not sure why we need to clear the screen twice
    // fillStatusLine(COLOR16M_DARKBLUE);
    selectScreen(PAGE2_START_ADDR); // Init page 2 screen
    fillScreen(COLOR16M_DARKBLUE);
    // fillStatusLine(COLOR16M_DARKBLUE);
...
boolean RA8876_common::ra8876SdramInitial(void) {
    ...
    lcdRegDataWrite(0xe2, Auto_Refresh);
    lcdRegDataWrite(0xe3, Auto_Refresh >> 8);
    lcdRegDataWrite(0xe4, 0x01); // 32-bit bus width
#endif
...
//**************************************************************//
//**************************************************************//
void RA8876_common::foreGroundColorbpp(ru32 color, bool finalize) {
...
//**************************************************************//
void RA8876_common::backGroundColorbpp(ru32 color, bool finalize) {
    lcdRegDataWrite(RA8876_BGCR, color >> 16, false);    // d5h
    lcdRegDataWrite(RA8876_BGCG, color >> 8, false);    // d6h
    lcdRegDataWrite(RA8876_BGCB, color << 0, finalize); // d7h
}
...
/* Read a 24bpp pixel                                           */
//**************************************************************//
ru32 RA8876_common::readPixel(int16_t x, int16_t y) {
    return getPixel(x, y);
}

ru32 RA8876_common::getPixel(ru16 x, ru16 y) {
    ru32 rdata = 0;
    ru32 dummy __attribute__((unused)) = 0;
    ...
    dummy = lcdDataRead();
    rdata = (lcdDataRead() & 0x000000ff); // read low byte
    rdata |= (lcdDataRead() << 8) & 0x0000FF00;    // add middle byte
    rdata |= (lcdDataRead() << 16) & 0x00FF0000;    // add high byte
...
/* Write a 24bpp pixel                                          */
//**************************************************************//
void RA8876_common::drawPixel(ru16 x, ru16 y, ru32 color) {
    graphicMode(true);
    setPixelCursor(x, y);
    ramAccessPrepare();

    lcdDataWrite((color >> 0), true);
    lcdDataWrite((color >> 8), true);
    lcdDataWrite((color >> 16), true);
}

void RA8876_common::readRect(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t *pcolors) {
...
//**************************************************************//
void RA8876_common::textColor(ru32 foreground_color, ru32 background_color) {
    check2dBusy();
    foreGroundColorbpp(foreground_color, false);
    backGroundColorbpp(background_color, true);
...
//**************************************************************//
void RA8876_common::clearStatusLine(uint32_t color) {
...
//**************************************************************//
void RA8876_common::writeStatusLine(ru16 x0, uint32_t fgcolor, uint32_t bgcolor, const char *str) {
    uint16_t tempX = _cursorX;
    uint16_t tempY = _cursorY;
    uint32_t tempBGColor = _TXTBackColor;
    uint32_t tempFGColor = _TXTForeColor;
...
//**************************************************************//
void RA8876_common::drawLine(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    y0 += _originy;
    y1 += _originy;
...
//**************************************************************//
void RA8876_common::drawSquare(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
...
// x1,y1 is lower right end corner
//**************************************************************//
void RA8876_common::drawSquareFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru32 color) {
...
// x1,y1 is lower right end corner
//**************************************************************//
void RA8876_common::drawSquareFill24(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru32 color) {
    //  Serial.printf("DSF:(%d %d)(%d %d) %x\n", x0, y0, x1, y1, color);
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
...
//**************************************************************//
void RA8876_common::drawCircleSquare(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 xr, ru16 yr, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
...
// yr is the minor radius of corner (vertical)
//**************************************************************//
void RA8876_common::drawCircleSquareFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 xr, ru16 yr, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    y1 += _originy;
    y1 += _originy;
...
//**************************************************************//
void RA8876_common::drawTriangle(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 x2, ru16 y2, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    x2 += _originx;
    y0 += _originy;
...
// x2,y2 is triangle end point
//**************************************************************//
void RA8876_common::drawTriangleFill(ru16 x0, ru16 y0, ru16 x1, ru16 y1, ru16 x2, ru16 y2, ru32 color) {
    x0 += _originx;
    x1 += _originx;
    x2 += _originx;
    y0 += _originy;
...
//**************************************************************//
void RA8876_common::drawCircle(ru16 x0, ru16 y0, ru16 r, ru32 color) {
    x0 += _originx;
    y0 += _originy;
...
// See page 59 of RA8876.pdf for information on drawing arc's. (1 of 4 quadrants at a time only)
//**************************************************************//
void RA8876_common::drawCircleFill(ru16 x0, ru16 y0, ru16 r, ru32 color) {
    x0 += _originx;
    y0 += _originy;
...
//**************************************************************//
void RA8876_common::drawEllipse(ru16 x0, ru16 y0, ru16 xr, ru16 yr, ru32 color) {
    x0 += _originx;
    y0 += _originy;
...
//**************************************************************//
void RA8876_common::drawEllipseFill(ru16 x0, ru16 y0, ru16 xr, ru16 yr, ru32 color) {
    x0 += _originx;
    y0 += _originy;
...
void RA8876_common::bteMemoryCopy(ru32 s0_addr, ru16 s0_image_width, ru16 s0_x, ru16 s0_y,
    ...
    lcdRegDataWrite(RA8876_BTE_COLR, (RA8876_S0_COLOR_DEPTH_24BPP << 5 | RA8876_S1_COLOR_DEPTH_24BPP << 2 | RA8876_DESTINATION_COLOR_DEPTH_24BPP) & 0x7f); // 92h
...
void RA8876_common::bteMemoryCopyWithChromaKey(
    ...
    bte_DestinationWindowStartXY(des_x, des_y);
    bte_WindowSize(copy_width, copy_height);
    backGroundColorbpp(chromakey_color);
...
void RA8876_common::bteMpuWriteColorExpansionData(ru32 des_addr, ru16 des_image_width, ru16 des_x, ru16 des_y, ru16 width,
    ...
    bte_DestinationWindowStartXY(des_x, des_y);
    bte_WindowSize(width, height);
    foreGroundColorbpp(foreground_color);
    backGroundColorbpp(background_color);
...
void RA8876_common::fillScreen(uint32_t color) {
...
void RA8876_common::fillStatusLine(uint32_t color) {
...
//**************************************************************//
void RA8876_common::setTextColor(uint32_t color) {
...
//**************************************************************//
//**************************************************************//
void RA8876_common::setBackGroundColor(uint32_t color) {
    backGroundColorbpp(color);
}

// Set text foreground + background colors
void RA8876_common::setTextColor(uint32_t fgc, uint32_t bgc) {
    if (_use_ili_font) {
...
//        textcolorPrexpanded = (fgc | (fgc << 24)) & 0x00FFFFFF;
//        textbgcolorPrexpanded = (bgc | (bgc << 24)) & 0x00FFFFFF;
    }
...
void RA8876_common::printStatusLine(uint16_t x0, uint32_t fgColor, uint32_t bgColor, const char *text) {
...
void RA8876_common::drawRect(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
void RA8876_common::fillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint32_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
void RA8876_common::fillRectVGradient(int16_t x, int16_t y, int16_t w, int16_t h,
    ...
    for (uint16_t j = h; j > 0; j--) {
        uint32_t color = RGB14tocolor565(r, g, b);
...
void RA8876_common::writeRect(int16_t x, int16_t y, int16_t w, int16_t h, const uint32_t *pcolors) {
    x += _originx;
    y += _originy;
...
void RA8876_common::writeRectImpl(int16_t x, int16_t y, int16_t w, int16_t h, const uint32_t *pcolors) {
	uint16_t start_x = (x != CENTER) ? x : (_width - w) / 2;  // Center X
	uint16_t start_y = (y != CENTER) ? y : (_height - h) / 2; // Center Y
...
void RA8876_common::putPicture(ru16 x, ru16 y, ru16 w, ru16 h, const uint32_t *data) {
    ...
    case 0:
        ...
        if (_bus_width != 16) {
            bteMpuWriteWithROPData8(currentPage, width(), start_x, start_y,       // Source 1 is ignored for ROP 12
                                    currentPage, width(), start_x, start_y, w, h, // destination address, pagewidth, x/y, width/height
                                    RA8876_BTE_ROP_CODE_12,
                                    (const uint32_t *)pcolors_aligned);
        } else {
            bteMpuWriteWithROPData16(currentPage, width(), start_x, start_y,       // Source 1 is ignored for ROP 12
                                     currentPage, width(), start_x, start_y, w, h, // destination address, pagewidth, x/y, width/height
                                     RA8876_BTE_ROP_CODE_12,
...
void RA8876_common::writeRect8BPP(int16_t x, int16_t y, int16_t w, int16_t h,
    ...
    uint32_t buffer[w] __attribute__((aligned(32)));
...
void RA8876_common::drawRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t xr, uint16_t yr, uint32_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
void RA8876_common::fillRoundRect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t xr, uint16_t yr, uint32_t color) {
    x += _originx;
    y += _originy;
    int16_t x_end = x + w - 1;
    int16_t y_end = y + h - 1;
...
// Draw a filled circle
void RA8876_common::fillCircle(int16_t x0, int16_t y0, int16_t r, uint32_t color) {
    drawCircleFill(x0, y0, r, color);
}

// Draw a filled ellipse.
void RA8876_common::fillEllipse(int16_t xCenter, int16_t yCenter, int16_t longAxis, int16_t shortAxis, uint32_t color) {
    drawEllipseFill(xCenter, yCenter, longAxis, shortAxis, color);
}

// Draw a filled triangle
void RA8876_common::fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1, int16_t x2, int16_t y2, uint32_t color) {
...
void RA8876_common::drawChar(int16_t x, int16_t y, unsigned char c,
                             uint32_t fgcolor, uint32_t bgcolor, uint8_t size_x, uint8_t size_y) {
    ...
    if (_backTransparent) {
        ...
        uint32_t char_buffer[w * h];
        uint32_t color;
        uint32_t *pfbPixel_row = char_buffer;
...
void RA8876_common::setFont(const ILI9341_t3_font_t &f) {
    ...
    if (font && font->version == 23) {
        ...
        if (_TXTForeColor == _TXTBackColor)
            _TXTBackColor = (_TXTForeColor == 0x00000000) ? 0xFFFFFFFF : 0x00000000;
    }
}
...
void RA8876_common::Pixel(int16_t x, int16_t y, uint32_t color) {
    x += _originx;
    y += _originy;
...
*/
/**************************************************************************/
void RA8876_common::drawFastVLine(int16_t x, int16_t y, int16_t h, uint32_t color) {
	x+=_originx;
	y+=_originy;
...
*/
/**************************************************************************/
void RA8876_common::drawFastHLine(int16_t x, int16_t y, int16_t w, uint32_t color) {
	x+=_originx;
	y+=_originy;
...
void RA8876_common::setRotation(uint8_t rotation) { // rotate text and graphics
    ...
    case 0:
        ...
        _portrait = false;
        VSCAN_T_to_B();
        macr_settings = RA8876_MASK_HIGH_BYTE_EVEN_DATA << 6 | RA8876_READ_MEMORY_LRTB << 4 | RA8876_WRITE_MEMORY_LRTB << 1;
        break;
    case 1:
        _portrait = true;
        ...
        VSCAN_B_to_T();
        macr_settings = RA8876_MASK_HIGH_BYTE_EVEN_DATA << 6 | RA8876_READ_MEMORY_LRTB << 4 | RA8876_WRITE_MEMORY_RLTB << 1;
        break;
    case 2:
        _width = SCREEN_WIDTH;
        _height = SCREEN_HEIGHT;
    ...
    case 3:
        ...
        macr_settings = RA8876_MASK_HIGH_BYTE_EVEN_DATA << 6 | RA8876_READ_MEMORY_LRTB << 4 | RA8876_WRITE_MEMORY_LRTB << 1;
        // VSCAN_T_to_B();
        // macr_settings = RA8876_MASK_HIGH_BYTE_EVEN_DATA<<6|RA8876_READ_MEMORY_LRTB<<4|RA8876_WRITE_MEMORY_BTLR<<1;
        break;
    }
    lcdRegWrite(RA8876_MACR); // 02h
...
void RA8876_common::MemWrite_Top_Down_Left_Right(void) {
    ...
    // temp = RA8876_MASK_HIGH_BYTE_EVEN_DATA<<6|RA8876_READ_MEMORY_BTLR<<4|RA8876_WRITE_MEMORY_BTLR<<1;
    temp = RA8876_MASK_HIGH_BYTE_EVEN_DATA << 6 | RA8876_READ_MEMORY_LRTB << 4 | RA8876_WRITE_MEMORY_TBLR << 1;
    //  Serial.println(temp, BIN);
    lcdRegDataWrite(RA8876_MACR, temp);
...
void RA8876_common::MemWrite_Down_Top_Left_Right(void) {
    ...
    // temp = RA8876_MASK_HIGH_BYTE_EVEN_DATA<<6|RA8876_READ_MEMORY_BTLR<<4|RA8876_WRITE_MEMORY_BTLR<<1;
    temp = RA8876_MASK_HIGH_BYTE_EVEN_DATA << 6 | RA8876_READ_MEMORY_LRTB << 4 | RA8876_WRITE_MEMORY_BTLR << 1;
    //  Serial.println(temp, BIN);
    lcdRegDataWrite(RA8876_MACR, temp);
